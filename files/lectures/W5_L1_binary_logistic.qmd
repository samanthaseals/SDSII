...
- where $\pi = \text{P}[Y = 1]$ = the probability of the outcome/event.
    ...
    - Recall, $\text{logit}()$ is the log-odds transformation. Here, $$\text{logit}(\pi) = \frac{\pi}{1-\pi}$$
...
- The Binomial distribution is a discrete probability distribution defined on the set $\{0, 1\}$.

- The Binomial distribution is characterized by $\pi$, the probability of success ($Y=1$), and $n$, the number of trials (sample size).
...
- Goofy and Max are gearing up for a huge performance weekend. To make sure he's ready, Max decides to test out a few different preparation plans leading up to the performance. Each plan mixes a different approach to practicing, resting, and getting through the day.
...
- Max is measuring the following:
	1.	Audition check: Max finds out whether he nailed the audition practice for that day.
	2.	Travel check: Goofy checks whether they made it to the concert venue on time for rehearsal, which is not guaranteed when Goofy is driving.
...
- Let's model whether Max nailed the audition (`nailed_audition`) based on the plan followed, hours of practice, and hours of sleep.    
...
$$ 
\ln \left( \frac{\hat{\pi}}{1-\hat{\pi}} \right) = -4.17 - 1.41 \text{ chaos} -0.66 \text{ Goofy} - 0.31 \text{ Max} + 0.65 \text{ practice} + 0.40 \text{ sleep}
$$
...
|:--|:--:|--:|
| variable 1 | OR_1_ (LL_1_, UL_1_) | *p*_1_|
| variable 2 | OR_2_ (LL_2_, UL_2_) | *p*_2_|
| ... | ... | ... |
| variable *k* | OR_*k*_ (LL_*k*_, UL_*k*_) | *p*_*k*_|
...
- Interpreting the slopes for *sleep hours*:
