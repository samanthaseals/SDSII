...
```{r}
#| eval: false
m <- glm(y = x1 + x2 + ... + xk, data = dataset_name)
...
```{r}
#| eval: false
full <- glm(y = x1 + x2 + ... + xk, data = dataset_name) # ALL predictors
reduced <- glm(y = x1 + ... + x(k-c), data = dataset_name) # all predictors WITHOUT indicators
anova(reduced, full)
```
...
```{r}
full <- glm(damage_cost ~ location + nephew, data = duck_incidents) # full model
reduced_loc <- glm(damage_cost ~ nephew, data = duck_incidents) # reduced without location
reduced_neph <- glm(damage_cost ~ location, data = duck_incidents) # reduced without nephew
...
```{r}
m4a <- glm(damage_cost ~ location + nephew, data = duck_incidents)
coefficients(m4a)
m4b <- glm(damage_cost ~ loc_garage + loc_kitchen + loc_living + nephew, data = duck_incidents)
coefficients(m4b)
```
